...
def retrieve_categories():
    categories_query = Category.query.order_by('id').all()
    categories = {}
  ...
  '''
    ...
    def create_question():
        try:
            ...
            abort(422)
# curl -d '{"question":"What is the answer to life, the universe & everything?", "answer":"42", "category":"1","difficulty":3}' -H "Content-Type: application/json" -X POST http://localhost:5000/questions
# curl -d '{"question":"the greatest showman", "answer":"Hugh Jackman",
# "category":"5", "difficulty":1}' -H "Content-Type: application/json" -X
...
# curl -d '{"searchTerm":"fyyur"}' -H "Content-Type: application/json" -X
...
# curl http://localhost:5000/categories/8/questions
    '''
  @TODO:
        ...
        try:
            ...
            for question in questions:
                if question.id not in previous_questions_id:
                    other_questions_id.append(question.id)

            if len(other_questions_id) == 0:
                new_question = None

            else:
                new_question_id = random.choice(other_questions_id)
            ...
            abort(422)
# curl -d '{"previous_questions": [2],"quiz_category":
# {"type":"Geography","id": "2"}}' -H 'Content-Type: application/json' -X
